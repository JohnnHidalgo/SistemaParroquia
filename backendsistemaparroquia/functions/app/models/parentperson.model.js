...
var parentPersonsModels = {
    ...
    getParentsByPerson: (data, callback) => {
        if (connection) {
            let sql = `select a.idparentperson, a.idperson, a.idparent, b.firstname, b.secondname, b.lastname, b.secondlastname from parentperson a, parent b where a.idperson = ${connection.escape(data.idperson)} and a.idparent = b.idparent and a.active = true`
            connection.query(sql, (error, rows) => {
                if (error) throw error
                callback(rows)
            })
